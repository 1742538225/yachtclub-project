...
import com.id0304.pay.unionpay.acp.sdk.DemoBase;
import com.id0304.pay.unionpay.acp.sdk.LogUtil;
import com.id0304.pay.unionpay.acp.sdk.SDKConfig;
...
@Service
public class YinLianPayServiceImpl implements PayAdapter {
    @Override
    public String pay(Consume consume, PaymentType paymentType) {
        ...
        requestData.put("orderId",consume.getOrderId());             //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
        requestData.put("txnTime", DemoBase.getCurrentTime());        //订单发送时间，取系统时间，格式为yyyyMMddHHmmss，必须取当前时间，否则会报txnTime无效
        requestData.put("currencyCode", "156");         			  //交易币种（境内商户一般是156 人民币）
        requestData.put("txnAmt", consume.getPrice()+"");             			      //交易金额，单位分，不要带小数点
